		...
		void render() override
		{
			...
			if (_shaderProgramLight->_shader)
			{
				...
				if (_spriteLight._uniforms)
				{
					...
					{
						_spriteLight._uniforms->light_direction->set(_camera.getCameraFront().x, _camera.getCameraFront().y, _camera.getCameraFront().z);
					}

					if (_spriteLight._uniforms->light_cutOff)
					{
						_spriteLight._uniforms->light_cutOff->set(glm::cos(glm::radians(_CutOff)));
					}
					
					if (_spriteLight._uniforms->light_outerCutOff)
					{
						_spriteLight._uniforms->light_outerCutOff->set(glm::cos(glm::radians(_OuterCutOff)));
					}
